...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1,final_sum=True):
...
class CNNLSTM_RGB_ENC(nn.Module):
    def __init__(self, pretrained=True,special_fd=128,temporal_fd = 128, stride_size = 3, head = 1):
        ...
        self.atten = MultiHeadedAttention(head,head*temporal_fd,0.4)
...
class HCN_Inertial_ENC(nn.Module):
    ...
    def __init__(self,
        ...
        self.atten = MultiHeadedAttention(head,head*feature_dim,0.4)
...
class multimo_classifier(nn.Module):
    def __init__(self,modal_num,class_num,feature_dim=128,h=2,mode='concat'):
        super(multimo_classifier,self).__init__()
        self.MAT = MultiHeadedAttention(h,feature_dim,0.4,final_sum=False)
        self.mode=mode
        if mode=="concat":
            self.norm = nn.Sequential(
                nn.BatchNorm1d(modal_num * feature_dim),
                nn.ReLU(),
                nn.Dropout(0.4),
            )
...
class Hamlet(nn.Module):
    def __init__(self, modal_list, class_num, feature_dim, m_head_num):
        super(Hamlet, self).__init__()
        self.UAT = HAMLET_ENC(modal_list)
        self.MAT = multimo_classifier(modal_num=len(modal_list), class_num=class_num, feature_dim=feature_dim, h=m_head_num)
    def forward(self, x):
        out = self.UAT(x)
        out = self.MAT(out)
        return out
...
if __name__ == '__main__':
    ...
    window_size = 5
    stride_size = 3
    model = Hamlet(["RGB","accIMU","gyroIMU"],37,128,2)
    model.eval()
